...
static void register_string_error_to_buffer(wchar_t** ppBufferDst, const WCHAR* pErrorText)
{
	free(*ppBufferDst);
	*ppBufferDst = NULL;

	if (pErrorText) {
		*ppBufferDst = _wcsdup(pErrorText);
	}
}
...
static void register_winapi_error(hid_device *pDev, const WCHAR *op)
{
	register_winapi_error_to_buffer(&pDev->last_error_str, op);
}

static void register_string_error(hid_device *pDev, const WCHAR* pErrorText)
{
	register_string_error_to_buffer(&pDev->last_error_str, pErrorText);
}
...
static void register_global_error(const WCHAR *pErrorText)
{
	register_string_error_to_buffer(&last_global_error_str, pErrorText);
}

static HANDLE open_device(const wchar_t *path, BOOL open_rw)
{
	DWORD nDesiredAccess = (open_rw)? (GENERIC_WRITE | GENERIC_READ): 0;
	...
	HANDLE handle = CreateFileW(path,
		...
		NULL,
		OPEN_EXISTING,
		FILE_FLAG_OVERLAPPED, // FILE_ATTRIBUTE_NORMAL( C++ use this )  FILE_FLAG_OVERLAPPED( async M-AI must use this! )
		0);
...
{
	if (hid_init() < 0) {  // register_global_error: global error is reset by hid_init
		return NULL;
	}

	struct hid_device_info* pRoot = NULL; // return object
	struct hid_device_info* pCurrent = NULL;
	...
	for (wchar_t* device_interface = device_interface_list; *device_interface; device_interface += wcslen(device_interface) + 1) {
		HANDLE device_handle = open_device(device_interface, TRUE);  // Open read-only handle to the device

		// Check validity of device_handle.
		if (device_handle == INVALID_HANDLE_VALUE) {  // Unable to open the device.
			continue;
		}
		...
		if ((vendor_id == 0x0 || attrib.VendorID == vendor_id) && (product_id == 0x0 || attrib.ProductID == product_id)) {
			...
			struct hid_device_info* pTmp = hid_internal_get_device_info(device_interface, device_handle);
...
void  HID_API_EXPORT HID_API_CALL hid_free_enumeration(struct hid_device_info* pDevices)
{
	/* TODO: Merge this with the Linux version. This function is platform-independent. */
	struct hid_device_info *p = pDevices;
	while (p) {
		struct hid_device_info* pNext = p->next;
		free(p->path);
		free(p->serial_number);
		free(p->manufacturer_string);
		free(p->product_string);
		free(p);
		p = pNext;
	}
}
...
{
	hid_device *dev = NULL;
	wchar_t* wzInterfacePath = NULL;
	HANDLE handleDevice = INVALID_HANDLE_VALUE;
	PHIDP_PREPARSED_DATA pp_data = NULL;
	HIDP_CAPS caps;
	...
	wzInterfacePath = hid_internal_UTF8toUTF16(path);
	if (!wzInterfacePath) {
	...
	handleDevice = open_device(wzInterfacePath, TRUE);
	...
	if (handleDevice == INVALID_HANDLE_VALUE) {
		...
		// handleDevice = open_device(wzInterfacePath, FALSE);  // MiMouse Hid device does not need this.(XCl 2024-12-17)

		/* Check the validity of the limited device_handle. */
		if (handleDevice == INVALID_HANDLE_VALUE) {
			register_global_winapi_error(L"open_device_");
			goto end_of_function;
		}
	}

	/* Set the Input Report buffer size to 64 reports. */
	if (!HidD_SetNumInputBuffers(handleDevice, 64)) {
		register_global_winapi_error(L"set input buffers");
		goto end_of_function;
	}

	/* Get the Input Report length for the device. */
	if (!HidD_GetPreparsedData(handleDevice, &pp_data)) {
	...
	dev = new_hid_device();
	if (dev == NULL) {  // impossible
		register_global_error(L"hid_device allocation error");
		goto end_of_function;
	}

	dev->device_handle = handleDevice;
	...
	dev->device_info = hid_internal_get_device_info(wzInterfacePath, dev->device_handle);

end_of_function:
	free(wzInterfacePath);
...
int HID_API_EXPORT HID_API_CALL hid_read_timeout(hid_device *dev, unsigned char *data, size_t length, int milliseconds)
{
	...
	HANDLE ev = dev->ol.hEvent;	 // Copy the handle for convenience.
	if (!dev->read_pending) {  // Start an Overlapped I/O read.
		...
		if (!res) {
			if (GetLastError() != ERROR_IO_PENDING) {  // ReadFile() has failed. Clean up and return error.
	...
	if (overlapped) {
		if (milliseconds >= 0) {
			res = WaitForSingleObject(ev, milliseconds);  // See if there is any data yet.
			if (res != WAIT_OBJECT_0) {
	...
	if (res && bytes_read > 0) {
		if (dev->read_buf[0] == 0x0) {
			...
			copy_len = length > bytes_read ? bytes_read : length;  // Copy the whole buffer, report number and all.
